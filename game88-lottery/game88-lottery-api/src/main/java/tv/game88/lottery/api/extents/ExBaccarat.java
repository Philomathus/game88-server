...
public class ExBaccarat {
    ...
    public static Map<String, Object> getBaccaratKillResult(Map<String, BigDecimal> prizeMap, Map<String, BigDecimal> betMap,
        ...
        if (bankerTotal > playerTotal) {
            if (bankWinMoney.compareTo(playerWinMoney) > 0) {
                ...
                //交换庄闲手牌
                resultMap.put("resultsList", Arrays.asList(String.join(",", bankerResultList), String.join(",", playerResultList)));
            } else {
            ...
            //和牌不杀
            resultMap.put("resultsList", Arrays.asList(String.join(",", playerResultList), String.join(",", bankerResultList)));
        }
    ...
    public static Map<String, Object> getBaccaratKillResult(Map<String, BigDecimal> prizeMap, Map<String, BigDecimal> betMap,
        ...
        // 先生成10组随机牌
        List<String> randomResult1 = ExBaccarat.randomResult();
        List<String> randomResult2 = ExBaccarat.randomResult();
        List<String> randomResult3 = ExBaccarat.randomResult();
        List<String> randomResult4 = ExBaccarat.randomResult();
        List<String> randomResult5 = ExBaccarat.randomResult();
        List<String> randomResult6 = ExBaccarat.randomResult();
        List<String> randomResult7 = ExBaccarat.randomResult();
        List<String> randomResult8 = ExBaccarat.randomResult();
        List<String> randomResult9 = ExBaccarat.randomResult();
        List<String> randomResult10 = ExBaccarat.randomResult();

        // 判断10组随机牌谁的派奖最少
        BigDecimal randomResultPrize1 = statisPrize(randomResult1, prizeMap, betMap);
        BigDecimal randomResultPrize2 = statisPrize(randomResult2, prizeMap, betMap);
        BigDecimal randomResultPrize3 = statisPrize(randomResult3, prizeMap, betMap);
        BigDecimal randomResultPrize4 = statisPrize(randomResult4, prizeMap, betMap);
        BigDecimal randomResultPrize5 = statisPrize(randomResult5, prizeMap, betMap);
        BigDecimal randomResultPrize6 = statisPrize(randomResult6, prizeMap, betMap);
        BigDecimal randomResultPrize7 = statisPrize(randomResult7, prizeMap, betMap);
        BigDecimal randomResultPrize8 = statisPrize(randomResult8, prizeMap, betMap);
        BigDecimal randomResultPrize9 = statisPrize(randomResult9, prizeMap, betMap);
        BigDecimal randomResultPrize10 = statisPrize(randomResult10, prizeMap, betMap);

        BigDecimal min = randomResultPrize1.min(randomResultPrize2).min(randomResultPrize3).min(randomResultPrize4).min(randomResultPrize5)
                .min(randomResultPrize6).min(randomResultPrize7).min(randomResultPrize8).min(randomResultPrize9).min(randomResultPrize10);
    ...
    public static BigDecimal statisPrize(List<String> randomResult, Map<String, BigDecimal> prizeMap, Map<String, BigDecimal> betMap) {
        BigDecimal bankerPrize = prizeMap.getOrDefault("庄", BigDecimal.ZERO);
        BigDecimal playerPrize = prizeMap.getOrDefault("闲", BigDecimal.ZERO);
        BigDecimal bankerPairsPrize = prizeMap.getOrDefault("庄对", BigDecimal.ZERO);
        BigDecimal playerPairsPrize = prizeMap.getOrDefault("闲对", BigDecimal.ZERO);
        BigDecimal tiePrize = prizeMap.getOrDefault("和", BigDecimal.ZERO);

        BigDecimal bankerBetAmount = betMap.getOrDefault("庄", BigDecimal.ZERO);
        BigDecimal playerBetAmount = betMap.getOrDefault("闲", BigDecimal.ZERO);

        String analyse = getBaccaratAnalyse(randomResult);

        BigDecimal total = BigDecimal.ZERO;
        switch (analyse) {
            case "闲":
                total = total.add(playerPrize);
                break;
            case "庄":
                total = total.add(bankerPrize);
                break;
            case "和":
                total = total.add(tiePrize).add(bankerBetAmount).add(playerBetAmount);
                break;
        }
        //闲对
        if(isPairs(randomResult.get(0))){
            total = total.add(playerPairsPrize);
        }
        //庄对
        if(isPairs(randomResult.get(1))){
            total = total.add(bankerPairsPrize);
        }
