...
@Repository( value = ConstantsPay.HAISI_PAY + "Processor" )
@Log4j2
public class HaiSiPayProcessor extends AbstractPay {
    @Override
    ...
    @Override
     public String orderPay( PayChannel payChannel, PayPlatform payPlatform, ReqPayRecharge reqPayRecharge ) throws Exception {
        ...
        reqMap.put( "pay_amount", reqPayRecharge.getMoney().setScale( 0, RoundingMode.HALF_UP ).toPlainString() );
        String string = this.assemblyUrl( reqMap ) + "&key=" + AESCoder.decrypt( payPlatform.getSignMd5() );
        String sign   = DigestUtils.md5Hex( string ).toUpperCase();
        reqMap.put( "pay_md5sign", sign );
        reqMap.put( "pay_productname", "test" );
        reqMap.put( "pay_post", "json" );
        reqMap.put( "pay_ip", reqPayRecharge.getRealIp() );

        Map<String, Object> resultMap = this.sendPostMap( payPlatform.getPayUrl(), packageForm( reqMap ), reqPayRecharge );
        ...
        if ( !CollectionUtils.isEmpty( resultMap ) ) {
            if ( "success".equals( resultMap.getOrDefault( "status", "" ).toString() ) ) {
                return resultMap.getOrDefault( "data", "" ).toString();
            } else {
    ...
    public boolean queryPay( MemberRechargeOnline memberRechargeOnline, PayPlatform payPlatform, PayChannel payChannel ) throws Exception {
        ...
        if ( !CollectionUtils.isEmpty( resultMap ) ) {
            ...
            if ( "SUCCESS".equals( state ) && "00".equals( code ) ) {
                ...
                memberRechargeOnline.setRealMoney( pay_amount.setScale( 0, RoundingMode.HALF_UP ) );
                return true;
            }
        }
        return false;
    ...
    public String callbackPay( Map<String, Object> requestMap, String realIp ) throws Exception {
        ...
        if ( StringUtils.equals( sign, mySign ) ) {
            ...
            if ( StringUtils.equals( "00", status ) && this.queryPay( memberRechargeOnline, payPlatform, payChannel ) ) {
                ...
                //              payJour.setSubMoney(pay_amount.setScale(0, BigDecimal.ROUND_HALF_UP));
