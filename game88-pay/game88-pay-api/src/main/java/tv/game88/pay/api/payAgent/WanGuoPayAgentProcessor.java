...
public class WanGuoPayAgentProcessor extends AbstractPayAgent {
    ...
    public boolean orderPay( MemberWithdrawDetail withdrawDetail, PayAgentChannel payAgentChannel,
        ...
        params.put( "amount", withdrawDetail.getWithdrawMoney().setScale( 2, RoundingMode.HALF_UP ) );
        params.put( "bankCode", bankCodeType.name() );
        params.put( "bankcardAccountNo", withdrawDetail.getBankAccount() );
        params.put( "bankcardAccountName", withdrawDetail.getBankUserName() );
        params.put( "notifyUrl", configEnvCacheUtil.getConf( "payAgentNotifyUrl" ) + payAgentPlatform.getCode() );

        String tempStr = this.assemblyUrl( params );
        params.put( "sign", RSACoder.signSha1Rsa( tempStr, AESCoder.decrypt( payAgentChannel.getSignPrivateKey() ) ) );
        log.warn( payAgentPlatform.getName() + "下单请求参数{}", JsonUtil.object2Json( params ) );

        Map<String, Object> resultMap = this.sendPostMap( payAgentPlatform.getOrderUrl(), packageJson( params ), reqPayAgent );
    ...
    public String callbackPay( PayAgentPlatform payAgentPlatform, Map<String, Object> requestMap, String realIp ) throws Exception {
        ...
        if ( RSACoder.verifySha1Rsa( tempStr, AESCoder.decrypt( payAgentChannel.getSignPublicKey() ), sign ) ) {
